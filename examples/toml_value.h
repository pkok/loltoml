...
namespace ConfigReader {
  ...
  public:
    ...
    TOMLValue(std::int8_t value) :
      type_(Type::integer),
      as_integer_(static_cast<std::int64_t>(value))
    {}
    ...
    TOMLValue(float value) :
      ...
      as_floating_point_(double(value))
    {}
     ...
     *
     * This will succeed if `type_` equals `TOMLValue::Type::integer`.
     ...
     * `std::domain_error`.
     *
     * Beware of overflow errors!  The internal representation is 64-bits,
     * so this can potentially hold larger values than you are trying to read.
     */
     ...
     * so this can potentially hold larger values than you are trying to read.
     * 
     * If `type_ == TOMLValue::Type::floating_point`, beware of rounding errors!
     * The internal representation is `double`, which is more precise.
     */
     ...
     * `std::domain_error`.
     *
     * If `type_ == TOMLValue::Type::integer`, beware of overflow errors!
     * The internal integer representation is 64-bits, which has a higher
     * maximum value than `double`.
     */
  ...
  private:
    ...
    /**
     * Get a string representation of the type_'s name.
     */
